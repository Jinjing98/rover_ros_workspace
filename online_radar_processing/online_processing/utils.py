...
def window_scalar(length, window_type):
    if window_type == Window.BARTLETT:
        window = np.sum(np.bartlett(length)) / length
    elif window_type == Window.BLACKMAN:
        window = np.sum(np.blackman(length)) / length
    elif window_type == Window.HAMMING:
        window = np.sum(np.hamming(length)) / length
    elif window_type == Window.HANNING:
        window = np.sum(np.hanning(length)) / length
    else:
        raise ValueError("The specified window is not supported!!!")
