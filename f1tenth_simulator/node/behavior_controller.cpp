...
public:
    BehaviorController() {
        ...
        brake_bool_sub = n.subscribe(brake_bool_topic, 1, &BehaviorController::brake_callback, this);  // the func will be called if sth is pubsed in
    ...
    void toggle_mux(int mux_idx, std::string driver_name) {
	    ...
	    //std::cout<<"the mux_idx is :"<<mux_idx<<std::endl;
    ...
    void brake_callback(const std_msgs::Bool & msg) {  //  to actually achive emergency braking, we need to call this funciton
    ...
    void key_callback(const std_msgs::String & msg) {
        ...
        if (msg.data == joy_key_char) {
            ...
            // emergency brake 
	    //toggle_mux(brake_mux_idx, "break"); ///jinjing
            ...
            // random walker
            toggle_mux(random_walker_mux_idx, "Random Walker");   // the launch file is running a random walker node keep publishing on topic /rand_drive
